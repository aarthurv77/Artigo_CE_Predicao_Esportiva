                ...
                if not found:
                    for key, value in metric_normalization_map.items():
                        if key.lower() in normalized_metric.lower():
                            ...
                            found = True
                            break
                if not found and len(normalized_metric) > 3 and 'cit' not in normalized_metric.lower():
...
def normalize_sport(value: str):
    """Normaliza o nome do esporte em categorias padrão."""
    if pd.isna(value):
        return None
    ...
    # Lixo / casos que queremos remover
    if ("não se aplica" in text_lower or "nao se aplica" in text_lower or
            "indústria" in text_lower or "industria" in text_lower):
        return None

    # Futebol
    if "futebol" in text_lower or "soccer" in text_lower or "european football" in text_lower:
        return "Futebol"

    # Basquete / NBA
    if "basket" in text_lower or "basquete" in text_lower or "nba" in text_lower:
        return "Basquete"

    # Tênis
    if "tênis" in text_lower or "tenis" in text_lower or "tennis" in text_lower:
        return "Tênis"

    # Hóquei
    if "hockey" in text_lower or "hóquei" in text_lower or "hoquei" in text_lower:
        return "Hóquei"

    # Futebol Americano / NFL
    if "futebol americano" in text_lower or "american football" in text_lower or "nfl" in text_lower:
    ...
    # Voleibol
    if "voleibol" in text_lower or "volleyball" in text_lower:
        return "Voleibol"

    # UFC / MMA
    if "mma" in text_lower or "ufc" in text_lower or "artes marciais mistas" in text_lower:
        return "UFC"

    # Se não bate em nada, devolve o texto original só pra não perder informação
    return text
...
try:
    ...
    df['Esporte_Normalizado'] = df['Esporte'].apply(normalize_sport)
    sport_counts = df['Esporte_Normalizado'].dropna().value_counts()
